...
int main(int argc, char* argv[])
{
  ...
  const int nbPhysicalParameters = 16 ;
  ...
  for(int k=2; k<thermal_treatment.size(); k++)
    temp_inp.push_back(thermal_treatment[k]) ;
  
  double t_end = time_temp.back();
...
vector<double> interpolate(double t, vector<double> time_stamps, vector<vector<double>> temp_stamps)
{
  ...
  if(t >= time_stamps.back())
    return temp_stamps.back();  
  else
  {
    int k=0 ;
